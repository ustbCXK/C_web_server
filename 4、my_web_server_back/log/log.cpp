...
{  
    m_count = 0;  
    m_mutex = new pthread_mutex_t; //创建互斥锁
    m_is_async = false;  
    pthread_mutex_init(m_mutex, NULL);  //初始化
}  
//析构函数，释放资源嘛
Log::~Log()  
{  
...
//日志分级的实现大同小异，一般的会提供五种级别：Debug、Warn、Info、Error、Fatal
//项目实现了前四个，使用了Debug，Info和Error
...
{  
    ...
    //my_tm.tm_mday为每次写日志的时候
    //m_today是创建文件的时候记录的时间
    //判断当前时间是不是今天，或者写入的日志行数是最大行的背书
